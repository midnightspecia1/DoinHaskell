...
--the difinition of the time series
data TS a = TS [Int] [Maybe a]

createTS :: [Int] -> [a] -> TS a
createTS times values = TS completTimes extendedValues
            where completTimes = [minimum times .. maximum times]
                  timeValueMap = Map.fromList (zip times values)
                  extendedValues = map (\v -> Map.lookup v timeValueMap) completTimes
...
--20.2 Stitching together TS with semigroup and monoids
...
--performing claculations on the time series
...
meanTS (TS times values) = if all (== Nothing) values
           ...
           where justVals = filter isJust values
                 fromJustValues = map fromJust justVals
                 average = mean fromJustValues
